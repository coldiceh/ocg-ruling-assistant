...
管理模型实验室用于在不改变公开问答路径的前提下，对同一问题、同一证据条件下的不同 GPT-5.6 模型和推理档位进行可复现实验。
...
`管理员登录 → 完整检索/廉价模型备料 → 冻结 Evidence Snapshot → GPT-5.6 最终裁定 → 确定性校验 → 保存结果与计量`

关键约束：

- DeepSeek 只能参与证据准备，不能输出最终裁定，也不能决定是否“升级”到 GPT。
- 每个管理实验 Run 都必须由允许列表中的 GPT-5.6 模型完成最终裁定。
...
仓库中的默认检查、单元测试和离线题集只使用 Mock 或本地资料，不调用付费模型。现阶段不会为了发布验证自动发起真实 DeepSeek 或 OpenAI 请求。只有管理员明确批准一次手工付费实验、在服务端配置凭据并显式打开上述两个开关后，才允许产生真实模型费用。可复制的安全起始配置见仓库根目录 `.env.example`。
...
- 给 GPT 的决策资料包默认硬限制为 120 KiB（按 UTF-8 JSON 字节计算），卡文/FAQ 正文、等价 ID、遗漏目录和冲突目录分别有独立上限。
- 包含题面、已解析卡片字段、资料包及外围元数据在内的最终 GPT 输入另有 512 KiB UTF-8 总字节硬边界；超出时失败关闭，不发送截断后可能改变语义的请求。这是输入安全边界，不是 Token、费用或运行时间预算。

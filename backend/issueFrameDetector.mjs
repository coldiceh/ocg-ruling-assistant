const frameDefinitions = [
  ...
  define("summon_response_window", /召唤成功时点|召唤成功时|召喚成功時|summon response window/iu, ["conditionText", "monsterEffects"], ["召唤成功响应窗口中是否存在可发动的效果"]),
  define("open_game_state_priority", /开放游戏状态|自由时点|进行.*1速|进行.*一速|open game state|turn player.*action/iu, ["conditionText"], ["响应窗口结束后由哪位玩家取得行动机会"]),
  define("copy_effect_activation_procedure", /复制.*(?:取对象|发动手续|发动条件)|同じ効果.*(?:対象|発動)|copy.*effect.*(?:target|activation procedure)/iu, ["conditionText", "spellTrapEffects"], ["copied effect 发动时必须完成的对象选择或手续"]),
  define("continuous_effect_during_resolution", /效果处理中.*(?:自坏|永续|持续)|处理过程中.*(?:墓地|场上).*变化|処理中.*永続|continuous effect.*during resolution/iu, ["continuousText", "monsterEffects"], ["当前处理完成前永续效果是否插入适用"]),
  define("spell_trap_card_activation_vs_effect_activation", /卡的发动.*效果发动|卡的发动.*[①②③]|カードの発動.*効果の発動|card activation.*effect activation/iu, ["conditionText", "spellTrapEffects"], ["当前只进行卡的发动，还是同时发动编号效果"]),

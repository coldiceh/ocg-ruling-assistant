...
export function validateEvidenceGenerationContract(contract) {
  ...
  if (contract.providerId !== 'gemini') throw new Error('evidence_generation_contract_provider_unsupported');
  ...
  if (!pricing || pricing.currency !== 'USD' || pricing.billingMode !== 'standard') {
  ...
  if (pricing.outputIncludesThinking !== true || pricing.cachedInputMode !== 'usage_reported'
      || pricing.requestCachingMode !== 'none') {
    throw new Error('evidence_generation_contract_invalid_pricing_categories');
  }
  return contract;
}

export function loadEvidenceGenerationContract(stage, { profileUrl = GEMINI_BASELINE_PROFILE } = {}) {
...
export function estimateGenerationUpperBoundUsd({ measurement, contract }) {
  ...
  const pricing = contract.pricingContract;
  const amountUsd = (input * pricing.inputUsdPerMillion
  ...
  return {
    ...
    basis: 'all_uncached_input_and_max_billable_output',
  };
}

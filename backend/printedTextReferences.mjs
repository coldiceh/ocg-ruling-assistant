...
const PRINTED_TEXT_COMPLEMENT = /(?:效果文本框|效果文本栏|效果文本里?|卡面|印刷文本|印刷文字|テキスト欄|カードテキスト|printed\s+(?:effect\s+)?text)[^，,。；;!?？]{0,32}?(?:记载|记述|記載|記述|写有|寫有|書かれ|mention)(?:有|了)?([^，,。；;!?？]{0,120})/iu;
const PRINTED_REFERENCE_REQUIREMENT_PATTERNS = Object.freeze([
  ...
  /[「『“"]([^「」『』“”"]{1,80})[」』”"]\s*(?:の)?\s*カード名が(?:記載|記述|記され|書かれ)/iu,
  /(?:カードテキスト|テキスト欄)\s*(?:に|へ)[^。！？]{0,32}?[「『“"]([^「」『』“”"]{1,80})[」』”"][^。！？]{0,20}?(?:記載|記述|書かれ)/iu,
  /(?:效果文本|卡片文本|卡面文本)(?:里|中|内)?[^，,。；;!?？]{0,32}?(?:写有|寫有|记载|记述)[^「『“"]{0,12}[「『“"]([^「」『』“”"]{1,80})[」』”"]/iu,
  /(?:mentions?|lists?|specifically lists?)\s+[「『“"]?([^「」『』“”",.;:]{2,80})[」』”"]?\s+(?:in (?:its|the) card text|as a card name)/iu,
  /(?:card|effect)\s+text[^,.;:]{0,32}?(?:mentions?|lists?|contains?)\s+[「『“"]([^「」『』“”"]{1,80})[」』”"]/iu,
]);

...
export function buildRagRulingPromptBundle({
  ...
  let prompt = [
    ...
    "必须严格区分规则条件与常见场面。官方资料若写明“只要/如果某条件成立就可以”，不得因为该条件在通常开局中不常见，就擅自假定题目中条件不成立并改答不能；题目未明确该事实时，应保留条件式结论。",
    ...
    "遇到‘送去墓地改为除外’等持续目的地替代时，必须按每次移动发生的时间点检查替代效果载体。发动时支付 cost 是独立的一次移动：若载体此时仍在适用区域，原定送墓的 cost 按替代后的实际目的地移动；不得因为载体稍后会离场而倒推 cost 不受替代。",
    "融合素材等多张卡在同一处理步骤同时移动时，必须把它们作为一个原子批次判断，不能按任意顺序逐张移动。如果目的地替代效果的载体自身也在这一批离开其适用区域，该替代不适用于这整个素材移动批次，其他同批素材也按原定目的地移动。",
    "这类题的结论必须同时写出：能否发动、cost 的实际去向、效果处理中同批素材的实际去向、最终是否完成融合召唤；不得只写‘可以发动’、‘融合正常’或遗漏 cost 与载体离场之间的时间差。",
    ...
    "同一时点有公开区域的选发诱发效果与手牌的诱发效果时，不得把两者直接当作同一玩家可任意排列的连锁。先宣言公开区域诱发，再把响应机会交给对方；对方不发动或响应权之后回到自己时，才可继续从手牌连锁发动。即使手牌因其他效果处于公开状态，发动位置仍是手牌，不得仅凭卡片已被看见就改入公开区域诱发顺序。",
    ...
    "当效果同时取多张卡为对象，而其中盖放的通常陷阱或速攻魔法在同一连锁发动时，必须依据该卡 FAQ 或效果连接关系判断：该对象无法执行位置移动后，其他对象和发动源是否继续处理；不得凭一般的尽可能处理原则猜测。",

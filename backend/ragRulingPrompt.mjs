...
function limitEvidence(items = [], limit, textLimit, label, warnings) {
  ...
  return source.slice(0, limit).map((item) => {
    ...
    return {
      ...
      ...(item.applicabilityReview
        ? { applicabilityReview: compactApplicabilityReview(item.applicabilityReview) }
        : {}),
    };
  });
}

function compactApplicabilityReview(review = {}) {
  const keep = (items, limit = 8) => (Array.isArray(items) ? items : [])
    .map((item) => String(item || "").replace(/\s+/gu, " ").trim().slice(0, 240))
    .filter(Boolean)
    .slice(0, limit);
  return {
    verdict: ["APPLICABLE", "INAPPLICABLE", "UNKNOWN"].includes(String(review.verdict || "").toUpperCase())
      ? String(review.verdict).toUpperCase()
      : "UNKNOWN",
    sharedConditions: keep(review.sharedConditions),
    missingConditions: keep(review.missingConditions),
    conflictingConditions: keep(review.conflictingConditions),
    reason: String(review.reason || "").replace(/\s+/gu, " ").trim().slice(0, 500),
  };
}

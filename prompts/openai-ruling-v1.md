...
对每个用户子问题分别给出 verdict，不得用一个结论代替多个问题。只检查与本题实际相关的规则、卡文条件和证据，不得因为固定题型清单而给题面添加额外问题或预设结论。

仅当当前子问题要求判断某次发动是否合法时，才审查发动时必须存在、选择或处理的合法候选；题面明确已经组成连锁且只问后续处理时，不得重新审查该次发动。若输入末尾另有 `legacyLuaPromptHints`，它只是独立实验模块自动生成的非权威核对提醒；只能提示模型回到题面、卡文和规则资料检查对应阶段、操作或筛选条件，不能单独证明任何条件成立或不成立，也不能作为 Evidence ID 或裁定依据。
...
`counterChecks` 只记录模型在本题中实际执行且确实相关的反向核对；没有相关核对时输出空数组。不要为了满足固定清单而添加与题面无关的规则类别。

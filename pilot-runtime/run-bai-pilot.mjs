...
function validateInput(input) {
 ...
 requireFact(Array.isArray(input.cells)&&input.cells.length===1,'unexpected_cell_count');
...
async function main() {
  ...
  const queues = Object.keys(ALIASES).flatMap(family => Array.from({length:1},(_,lane)=>input.cells.filter(c=>familyOf(c.request.model)===family).filter((_,i)=>i%1===lane)));

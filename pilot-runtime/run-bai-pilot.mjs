...
function validateInput(input) {
 ...
 requireFact(Array.isArray(input.cells)&&input.cells.length===2,'unexpected_cell_count');
...
export async function invoke(request, apiKey, cellStarted, idleTimeoutMs = 1_800_000) {
  ...
  const state = { startedAt: new Date().toISOString(), httpStatus: null, content: '', reasoningChars: 0, usage: null,
    ...
    timeoutPolicy: 'absolute_30_minutes_for_max_thinking', totalTimeoutMs: idleTimeoutMs };
  const consume = block => {

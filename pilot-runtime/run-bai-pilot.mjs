...
function validateInput(input) {
 requireFact(input?.schemaVersion===1 && typeof input.runId==='string','invalid_run_contract');
 requireFact(Array.isArray(input.cells)&&input.cells.length===98,'unexpected_cell_count');
 requireFact(new Set(input.cells.map(c=>c.cellId)).size===input.cells.length,'duplicate_cell');
 requireFact(new Set(input.cells.map(c=>c.request.model+':'+c.group+':'+c.questionId)).size===input.cells.length,'duplicate_model_question_group');
 for(const c of input.cells){const r=c.request,f=familyOf(r?.model);
 requireFact(Boolean(f)&&['F','Q'].includes(c.group),'invalid_model_or_group');
 requireFact(/^[a-zA-Z0-9_-]{1,100}$/.test(c.cellId),'invalid_cell_id');
 requireFact(r.messages?.length===1&&r.messages[0].role==='user'&&typeof r.messages[0].content==='string','one_user_required');
 requireFact(Object.keys(r).sort().join(',')===['model','messages','reasoning_effort','thinking','max_tokens','stream','stream_options'].sort().join(','),'unexpected_request_fields');
 requireFact(r.reasoning_effort==='max'&&r.thinking?.type==='enabled','thinking_changed');
 requireFact(r.max_tokens===({deepseek:393216,glm:131072})[f],'model_capacity_changed');
 requireFact(r.stream===true&&r.stream_options?.include_usage===true,'stream_usage_required');
 }
}
...
export async function invoke(request, apiKey, cellStarted, idleTimeoutMs = 3_600_000) {
  ...
  markNetworkActivity();
  const state = { startedAt: new Date().toISOString(), httpStatus: null, content: '', reasoningChars: 0, usage: null,
    ...
    timeoutPolicy: 'absolute_60_minutes_for_max_thinking', totalTimeoutMs: idleTimeoutMs };
  const consume = block => {
    ...
    for (const choice of payload.choices || []) {
      ...
      state.reasoningChars += (choice.delta?.reasoning_content ?? choice.message?.reasoning_content ?? '').length;
...
async function main() {
  ...
  const queues = Object.keys(ALIASES).flatMap(family => Array.from({length:20},(_,lane)=>input.cells.filter(c=>familyOf(c.request.model)===family).filter((_,i)=>i%20===lane)));
  ...
  for (let index = 0; index < outcomes.length; index++) {
    ...
    if (outcome.status === 'rejected') summary.infrastructureErrors.push({ queue: index, code: outcome.reason?.code || 'local_checkpoint_failure' });
  }

...
function validateInput(input) {
  ...
  for (const cell of input.cells) {
    ...
    // Mechanical request-contract check only. No lower trial cap or implicit lower provider default.
    const capacity = { astra: 128000, deepseek: 393216, glm: 131072 }[family];
    const capacityField = family === 'astra' ? 'max_completion_tokens' : 'max_tokens';
    requireFact(request[capacityField] === capacity, 'documented_provider_capacity_required');
    requireFact(['max_tokens', 'max_completion_tokens', 'max_output_tokens']
      .every(field => field === capacityField || request[field] === undefined), 'conflicting_output_limit');
  }
}
...
export async function invoke(request, apiKey, cellStarted, idleTimeoutMs = 300_000) {
  ...
  const state = { startedAt: new Date().toISOString(), httpStatus: null, content: '', usage: null,
    ...
    timeoutPolicy: 'continuous_network_idle', idleTimeoutMs };
  const consume = block => {
  ...
  try {
    ...
    } else state.errorCode = idleTimedOut ? 'idle_timeout_outcome_unknown' : (error.code || 'transport_or_stream_failure');

...
test("synthetic protocol records produce source-bound queries", () => {
  const result = planFixture(protocolFixture);
  assert.equal(result.kind, "READY");
  assert.deepEqual(result.scenario.queries.map((query) => query.queryId), ["query-a", "query-b"]);
  ...
  assert.equal(result.scenario.definitionSnapshot.snapshotId, "fixture:synthetic-definitions:v1");
...
test("ambiguous card binding remains UNKNOWN instead of guessing a definition", () => {
  const input = materializePlannerFixture(protocolFixture);
...
test("card instance ids cannot be detached from their exact resolved-card mentions", () => {
  const swappedIds = materializePlannerFixture(protocolFixture);
  ...
  const swappedSpans = materializePlannerFixture(protocolFixture);
  ...
  const collidingAlias = materializePlannerFixture(protocolFixture);
...
test("explicit numbered effects bind to the named card instead of draft-reported effect ids", () => {
  const swappedEffect = materializePlannerFixture(protocolFixture);
  ...
  const swappedSubject = materializePlannerFixture(protocolFixture);
  ...
  const eventMismatch = materializePlannerFixture(protocolFixture);
  ...
  const intentMismatch = materializePlannerFixture(protocolFixture);
...
test("chain-order effect bindings preserve textual order and duplicate occurrences", () => {
  const ordered = materializePlannerFixture(protocolFixture);
  ...
  const duplicated = materializePlannerFixture(protocolFixture);
...
test("an unquoted short alias cannot bind inside a longer resolved card name", () => {
  const embedded = materializePlannerFixture(protocolFixture);
  ...
  const standalone = materializePlannerFixture(protocolFixture);
...
test("entity references bind to an exact card mention inside their own source span", () => {
  const multiMention = materializePlannerFixture(protocolFixture);
...
test("missingStateFacts is a required array of non-empty strings", () => {
  const valid = materializePlannerFixture(protocolFixture);
...
test("missing strict state and authored engine conclusions remain UNKNOWN", () => {
  const missing = materializePlannerFixture(protocolFixture);
  ...
  const authored = materializePlannerFixture(protocolFixture);
  ...
  const authoredType = materializePlannerFixture(protocolFixture);
  ...
  for (const mutate of [
    ...
    const closedSchema = materializePlannerFixture(protocolFixture);
...
test("input provenance, references, and immutable definition snapshots fail closed", () => {
  const missingProvenance = materializePlannerFixture(protocolFixture);
  ...
  const injectedConclusion = materializePlannerFixture(protocolFixture);
  ...
  const wrongProvenance = materializePlannerFixture(protocolFixture);
  ...
  const unknownEvent = materializePlannerFixture(protocolFixture);
  ...
  const badReference = materializePlannerFixture(protocolFixture);
  ...
  const cyclicQueries = materializePlannerFixture(protocolFixture);
  cyclicQueries.draft.queries[0].dependsOn = ["query-b"];
  ...
  const printedReference = materializePlannerFixture(protocolFixture);
  ...
  const mixedSnapshot = materializePlannerFixture(protocolFixture);

...
const fixture = JSON.parse(await readFile(new URL("./fixtures/formal-engine/synthetic-protocol.json", import.meta.url), "utf8"));
...
test("top-level artifact binding failures are global while q2 verifier failure stays local", async () => {
  ...
  const localResponse = await requestFormalScenarioAnalysis({
    ...
    proofVerifier: (input) => {
      if (input.queryResult.queryId === "query-b") throw new Error("q2 verifier rejected proof");
...
test("async proof verifiers are awaited and rejected promises downgrade only their query", async () => {
  ...
  assert.deepEqual(verifiedQueries.sort(), ["query-a", "query-b"]);
  ...
  const rejected = await requestFormalScenarioAnalysis({
    ...
    proofVerifier: async (input) => {
      await Promise.resolve();
      if (input.queryResult.queryId === "query-b") throw new Error("asynchronous verifier rejection");
      return mockPublicProofVerifier(input);
    },
  });
...
test("ABSTRACT_ASSUMPTIONS cannot retain TRUE or FALSE at the client boundary", async () => {
  ...
  abstractScenario.assumptions = [{
    assumptionId: "assume-player-value",
    type: "ASSUME_STATE_FACT",
    assumesFactId: "player-value",

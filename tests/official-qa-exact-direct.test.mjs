...
test("HTML, Unicode width, punctuation and whitespace variants retain the same exact identity", async () => {
  const base = fixture.positiveCases.find((item) => item.qaId === "910006").question;
  const variant = `  ${base
    .replaceAll("「", "&quot;")
    .replaceAll("」", "&#34;")
    .replaceAll("？", "?")
  ...
  assert.equal(result.qaId, "910006");
  assert.equal(result.route, "official_qa_exact_direct");
});
...
test("an explicit Chinese qaData question surface matches the same current official Q&A", async () => {
  ...
  const cards = fixture.cards.map((card) => {
    if (card.id === "900007") return { ...card, cnName: "合成卡G", aliases: [...card.aliases, "合成卡G"] };
    if (card.id === "900008") return { ...card, cnName: "合成卡H", aliases: [...card.aliases, "合成卡H"] };
    return card;
  });
  const chineseQuestion = "「合成卡G」与「合成卡H」的记录五对应哪个字段？";
  const result = await retrieveExactOfficialQaDirect({
  ...
  assert.equal(result.status, "matched");
  assert.equal(result.route, "official_qa_exact_direct");
  assert.equal(result.qaId, "910005");
...
test("same-card different official questions remain distinct", async () => {
  ...
  assert.equal(result.qaId, "910003");
  assert.notEqual(result.qaId, "910004");
...
test("a discovered Q&A whose body is missing locally is fetched only by its explicit candidate ID", async () => {
  const targetIds = new Set(["910003", "910004"]);
  const calls = [];
  ...
  const item = fixture.positiveCases.find((entry) => entry.qaId === "910004");
  const result = await retrieveExactOfficialQaDirect({
    question: item.question,
    cards: fixture.cards,
  ...
  assert.equal(result.qaId, "910004");
  assert.deepEqual(
    calls.map((url) => url.match(/\/data\/qa\/(\d+)$/u)?.[1]).sort(),
    ["910003", "910004"],
  );
  assert.equal(result.officialAnswerJapanese, materialize(recordsById.get("910004").rawAnswer));
});

test("a newly updated multi-card Q&A body is fetched by the unique discovery intersection", async () => {
  const target = recordsById.get("910006");
  const calls = [];
  const fetchImpl = async (url) => {
    calls.push(String(url));
    if (!String(url).endsWith("/data/qa/910006")) throw new Error(`unexpected URL ${url}`);
    return jsonResponse({
      cards: [900009, 900010, 900011],
      qaData: { ja: {
        id: 910006,
        title: target.title,
  ...
  const item = fixture.positiveCases.find((entry) => entry.qaId === "910006");
  const result = await retrieveExactOfficialQaDirect({
    question: item.question,
    cards: fixture.cards,
...
test("a unique discovered Q&A with an unavailable body fails closed before every model stage", async () => {
  ...
  await assert.rejects(
    answerRagRulingQuestion({
      ...
      officialQaDiscovery: {
        ...fixture.discovery,
        exactQuestionIdentities: [{ qaId: "910006", questionHash }],
      },
      fetchImpl: async () => {
...
test("one card-intersection candidate without a question hash is not treated as an exact identity", async () => {
  const result = await retrieveExactOfficialQaDirect({
    question: "「合成カードI」や「合成カードJ」が存在する別の状況について教えてください。",
    cards: fixture.cards,
    qaRecords: fixture.records.filter((record) => record.sourceId !== "910006"),
    qaDiscovery: fixture.discovery,
    fetchImpl: async () => {
...
test("a legacy compact snapshot can recover its complete question and answer without a model", async () => {
  const current = recordsById.get("910001");
  ...
  const item = fixture.positiveCases.find((entry) => entry.qaId === "910001");
  const result = await retrieveExactOfficialQaDirect({
    question: item.question,
    cards: fixture.cards,
    qaRecords: [compact],
  ...
  assert.equal(result.qaId, "910001");
  assert.equal(result.officialQuestionJapanese, materialize(current.rawDetailedQuestion));
  assert.equal(result.officialAnswerJapanese, materialize(current.rawAnswer));
});

test("an extra decisive condition cannot inherit the shorter official question's direct route", async () => {
  const exact = fixture.positiveCases.find((item) => item.qaId === "910004").question;
  const result = await retrieveExactOfficialQaDirect({
    question: `${exact}\n追加の入力記録「合成カードG」があります。`,
    cards: fixture.cards,
    qaRecords: fixture.records,
    qaDiscovery: fixture.discovery,
    fetchImpl: neverFetch,
...
test("an ambiguous card alias never certifies an exact route", async () => {
  ...
  const ambiguousCards = [
    ...fixture.cards,
    { id: "999999", jaName: "合成カードG", aliases: ["合成カードG"] },
  ];
  const result = await retrieveExactOfficialQaDirect({
    question: materialize(current.rawDetailedQuestion),
...
test("pipeline exact hit bypasses card extraction, rule query, validator and final model even with unusable Relay settings", async () => {
  const item = fixture.positiveCases.find((entry) => entry.qaId === "910005");
  let modelCalls = 0;
  const rejectModelCall = async () => {
    modelCalls += 1;
  ...
  assert.equal(answer.officialQaId, "910005");
...
test("exact-only pipeline miss returns without invoking any model", async () => {
  ...
  const answer = await answerRagRulingQuestion({
    question: "「合成カードG」の似ているが公式原文ではない質問ですか？",
    cards: fixture.cards,
    records: [],
    qaRecords: fixture.records,
    officialQaDiscovery: fixture.discovery,

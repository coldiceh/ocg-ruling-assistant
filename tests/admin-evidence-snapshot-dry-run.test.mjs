...
const syntheticCases = {
  ...
  cases: Array.from({ length: 4 }, (_, index) => ({
    ...
    question: `请读取匿名测试卡${index}的效果文本。`,
    candidateCards: [`匿名测试卡${index}`],
  })),
};
const syntheticGoldens = {
  goldens: syntheticCases.cases.map(({ id }) => ({
    id, leakCanary: `DO_NOT_EXPOSE_${id}`, expectedAnswer: `PRIVATE_RESULT_${id}`,
  })),
};
const loadSyntheticData = async () => ({
  cards: syntheticCases.cases.map((item, index) => ({
    id: String(990000 + index), name: item.candidateCards[0], cnName: item.candidateCards[0],
    cardType: "效果怪兽", effectText: "①：自己主要阶段可以发动。抽1张卡。",
    aliases: [item.candidateCards[0]],
  })),
  records: [], qaRecords: [], rules: [],
});

test("synthetic fixture contains only input fields", async () => {
  const fixture = structuredClone(syntheticCases);
...
test("helper and CLI contain no case-specific ids or card-name branches", async () => {
  const cases = normalizeAdminEvidenceDryRunCases(syntheticCases);
...
test("four local cases freeze snapshots, never leak goldens, and never use a paid transport", async () => {
  const cases = normalizeAdminEvidenceDryRunCases(syntheticCases);
  const goldenFixture = structuredClone(syntheticGoldens);
  const artifacts = new Map();
  const result = await runAdminEvidenceSnapshotDryRun({
    loadData: loadSyntheticData,
    cases,
...
test("CLI leaves the Lua factory disabled for the default pure-LLM dry run", async () => {
  let factoryCalls = 0;
  let dryRunOptions = null;
  await runAdminEvidenceDryRunCli(["--cases", "synthetic.json", "--compact"], {
    readCases: async () => ({
      schemaVersion: 1,
...
test("an injected local Lua packet remains UNKNOWN and cannot enable paid transport", async () => {
  const fixture = normalizeAdminEvidenceDryRunCases(syntheticCases);
  const oneCase = {
...
test("dry-run CLI requires an explicit cases file before reading input", async () => {
  let reads = 0;
  await assert.rejects(() => runAdminEvidenceDryRunCli([], {
    readCases: async () => { reads += 1; return syntheticCases; },
    stdout: { write() {} },
  }), /--cases is required/u);

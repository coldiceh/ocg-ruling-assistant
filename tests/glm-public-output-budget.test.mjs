...
async function invoke(effort, overrides={}, finish='stop') {
  ...
  const result=await callRagModel({prompt:'测试请求',env,outputMode:'plain_text',fetchImpl:async(url,options)=>{
    ...
    return new Response(JSON.stringify({model:'glm-5.3-flash',choices:[{finish_reason:finish,message:{content:finish==='stop'?'已完成测试正文。':'',reasoning_content:'private reasoning'}}],usage:{prompt_tokens:10,completion_tokens:20,total_tokens:30,completion_tokens_details:{reasoning_tokens:10}}}),{status:200,headers:{'content-type':'application/json'}});
  }});
...
for(const effort of ['low','high','max'])test(`GLM ${effort} keeps room for thinking and final answer despite legacy 4096`,async()=>{
  ...
  assert.equal(calls,1);assert.equal(body.model,'glm-5.3-flash');assert.equal(body.reasoning_effort,effort);
  assert.deepEqual(body.thinking,{type:'enabled'});assert.equal(body.max_tokens,131072);
  assert.equal(result.generationConfig.maxOutputTokens,131072);
  assert.equal(result.generationAttempts[0].maxOutputTokens,131072);
...
test('GLM Flash uses full provider capacity despite legacy output caps',async()=>{
  assert.equal((await invoke('max',{GLM_THINKING_MAX_OUTPUT_TOKENS:'70000',RAG_FLASH_THINKING_MAX_OUTPUT_TOKENS:'80000'})).body.max_tokens,131072);
  assert.equal((await invoke('max',{RAG_FLASH_THINKING_MAX_OUTPUT_TOKENS:'80000'})).body.max_tokens,131072);
});

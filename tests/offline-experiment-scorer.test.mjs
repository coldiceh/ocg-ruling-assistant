...
const syntheticAssertionFixture = () => ({
  schemaVersion: 1,
  goldens: ["synthetic-match", "synthetic-other"].map((id) => ({
    id, assertions: [{ id: "literal-marker", description: "Synthetic marker transport check.", allOf: [["SYNTHETIC_MATCH"]] }],
  })),
});

test("scores a successful Sol result PASS and a timed-out result INCONCLUSIVE without artifact files", async () => {
  const assertionFixture = syntheticAssertionFixture();
  const successfulReport = batchReport([succeededCase("synthetic-match", {
    conciseAnswer: "SYNTHETIC_MATCH",
  })]);
  const failedReport = batchReport([{
    caseId: "synthetic-other",
    results: [{
      status: "FAILED",
...
test("validated structured result is preferred over a conflicting matrix summary", async () => {
  const assertionFixture = syntheticAssertionFixture();
  const item = succeededCase("synthetic-match", {
    conciseAnswer: "SYNTHETIC_MATCH",
  }).results[0];
  item.conciseAnswer = "SYNTHETIC_MISMATCH";
  const scored = scoreOfflineExperimentReport({
    report: batchReport([{ caseId: "synthetic-match", results: [item] }]),
    assertionFixture,
  });
  assert.equal(scored.results[0].status, "PASS");
...
test("scores the local single-process checkpoint format after transport completion", async () => {
  ...
  const report = {
    ...
    plannedRequests: 1,
    results: [{
      key: "synthetic-match:sol:low:full",
      requestId: "request-123",
      finalInputSha256: "input-sha-123",
      caseId: "synthetic-match",
      ...
      rawOutput: "{\"conciseAnswer\":\"SYNTHETIC_RAW\"}",
      validatedResult: {
        ok: true,
        normalized: {
          conciseAnswer: "SYNTHETIC_MATCH",
  ...
  assert.deepEqual(scored.results[0].sourceBinding, {
    status: "bound",
    resultKey: "synthetic-match:sol:low:full",
    requestId: "request-123",
    finalInputSha256: "input-sha-123",
    rawOutputSha256: sha256("{\"conciseAnswer\":\"SYNTHETIC_RAW\"}"),
    unavailableReasons: [],
  });
...
test("direct Relay terminal outcomes retain exact source bindings across three execution states", async () => {
  ...
  const validRaw = "{\"conciseAnswer\":\"SYNTHETIC_MATCH\"}";
  const report = {
    schemaVersion: 1,
    runner: "local-relay-effort-experiment/v1",
    status: "completed",
...
test("source bindings hash exact string and JSON-stringified object outputs", async () => {
  ...
  const reports = [
    ...
    succeededCase("synthetic-match", {
      conciseAnswer: "SYNTHETIC_MISMATCH",
    }, "low", {
      key: "same-logical-identity",
...
test("legacy admin results stay scoreable but cannot masquerade as review-bound output", async () => {
  const assertionFixture = syntheticAssertionFixture();
  const report = batchReport([succeededCase("synthetic-match", {
    conciseAnswer: "SYNTHETIC_MATCH",
  })]);
...
test("assertion fixture schema validation rejects invalid input", async () => {
  assert.throws(
...
test("scoring CLI requires explicit assertions before reading any report", async () => {
  let reads = 0;
  await assert.rejects(() => scoreAdminModelExperimentFiles({
    reportFile: "report.json",
    readFileImpl: async () => { reads += 1; return "{}"; },
  }), /--assertions is required/u);

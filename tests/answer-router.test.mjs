...
test("exact official QA cannot answer before canonical card identity parsing", async () => {
  ...
  assert.equal(answer.status, "needs_card_confirmation");
  assert.equal(answer.verdict, "needs_card_confirmation");
...
test("copied targeting prose without an effect instance stays conditional", async () => {
  ...
  assert.equal(answer.answerType, "needs_clarification");
  assert.equal(answer.answerRoute, "conditional_branch_answer");
  assert.equal(answer.verdict, "insufficient_for_single_verdict");
  assert.notEqual(answer.verdict, "copied_effect_selects_target_on_activation");
});

test("continuous self-destruction prose without an executable timeline stays insufficient", async () => {
  const answer = await answerRulingQuestionFast({
  ...
  assert.equal(answer.verdict, "insufficient");
  assert.notEqual(answer.verdict, "continuous_effect_checked_after_current_resolution");
});

test("card-activation and numbered-effect prose stays conditional without bound effects", async () => {
  const answer = await answerRulingQuestionFast({
  ...
  assert.equal(answer.answerRoute, "conditional_branch_answer");
  assert.equal(answer.verdict, "insufficient_for_single_verdict");
  assert.notEqual(answer.verdict, "card_activation_does_not_imply_numbered_effect_activation");
});

...
test("full-cache identity planning skips non-OCG corpus records without aborting", async () => {
  ...
  const resolved = await resolveLegacyLuaCardIdentityBatches({
    cards: [
      { id: "900101", name: "匿名卡甲", aliases: ["匿名カード甲"] },
      {
        id: "-75",
        type: "skill",
        cardType: "skill",
        name: "Synthetic Skill",
        aliases: ["Synthetic Skill"],
      },
    ],
  ...
  assert.deepEqual(requests.map((item) => item.clientKey), ["cid-900101"]);
  assert.equal(resolved.length, 1);
  assert.equal(resolved[0].cid, "900101");
});
...
test("cache-bound alias ordering is identical for opposing input orders", () => {
  ...
  const makeShard = (orderedAliases) => createPrecomputedLegacyLuaCacheShard({
    shardId: precomputedLegacyLuaShardId("90000001"),
    generatedAt: GENERATED_AT,
    entries: [{
      cid: "900102",
      passcode: "90000001",
...
test("every synthetic shard matches its manifest descriptor after serialization", () => {
  const fixture = makeFixture();
  const manifest = validatePrecomputedLegacyLuaCacheManifest(fixture.manifest);
  assert.equal(manifest.shards.length, 2);
  let entryCount = 0;
  for (const descriptor of manifest.shards) {
    const serialized = JSON.stringify(fixture.shards.get(descriptor.shardId));
    assert.equal(Buffer.byteLength(serialized, "utf8"), descriptor.serializedBytes);
    const shard = validatePrecomputedLegacyLuaCacheShard(JSON.parse(serialized), {

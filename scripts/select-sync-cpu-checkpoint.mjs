...
export function parseCpuCheckpointLog(raw) {
  ...
  if (sequences.length > 1) fail('sync_checkpoint_sequence_ambiguous');
  ...
  return {schemaVersion:1,status:'paused_time',totalRows:cachedRows+pendingRows,
...
export async function selectCheckpoint({repo,currentRunId,workspace,temp,runId='',sequence=0}) {
  ...
  if(run.conclusion !== 'failure' || job?.steps?.find(s=>s.name===cpuStep)?.conclusion !== 'failure') {
    if(sequence || process.env.SYNC_RESUME_RUN_ID) fail('sync_requested_checkpoint_not_cpu_pause');
  ...
  if(changed) {
    if(process.env.SYNC_RESUME_RUN_ID) fail('sync_checkpoint_published_base_changed');
    return fresh();
  }
